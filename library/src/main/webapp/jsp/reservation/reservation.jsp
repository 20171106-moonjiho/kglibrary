...
<body>
	...
	<div id="cont_wrap">
		<form action="reservation" method='post'>
			날짜 <input type="text" id="startDate" name="reservation_date">
			시간 <select name="reservation_time">
		...
		<c:if test="${not empty dto }">
			<form action="reservation2" method='post' id='f'>
						...
						<div class="btn-group" role="group"
							...
							<c:forEach var="s_no" begin="1" end="3">
								<input type="radio" class="btn-check" autocomplete="off"
									name="room_num" id="${seat_class }${s_no }"
									value="${seat_class }${s_no }">
								<label class="btn btn-outline-primary"
									for="${seat_class }${s_no }" id="${seat_class }${s_no }_lb">${seat_class}${s_no }</label>
							</c:forEach>
						</div>
						<br>
						<br>
				...
				<input type="hidden" name="reservation_time"
					value="${dto.reservation_time }"> <input type="hidden"
					name="reservation_date" value="${dto.reservation_date }"> <input
